...
const Dashboard = () => {
  ...
  // Floors & Tables Plan Editor states
    ...
    <div style={dashboardContainerStyle}>
      ...
      <div style={contentAreaStyle}>
        ...
        <main style={mainContentStyle}>
          ...
          {/* Floors & Tables Plan Editor Section */}
          <div style={{ marginTop: '20px' }}>
            <h2 style={sectionTitleStyle}>Floors & Tables Plan Editor</h2>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', backgroundColor: 'var(--input-bg)', padding: '4px', borderRadius: '8px' }}>
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: '700',
                      cursor: 'pointer',
                      backgroundColor: adminActiveFloor === 1 ? 'var(--border-focus)' : 'transparent',
                      color: adminActiveFloor === 1 ? 'var(--bg-primary)' : 'var(--text-secondary)',
                      transition: 'all 0.2s'
                    }}
                  >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: '700',
                      cursor: 'pointer',
                      backgroundColor: adminActiveFloor === 2 ? 'var(--border-focus)' : 'transparent',
                      color: adminActiveFloor === 2 ? 'var(--bg-primary)' : 'var(--text-secondary)',
                      transition: 'all 0.2s'
                    }}
                  >
                    Rooftop (Floor 2)
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <input
                    type="text"
                    placeholder="Table name (e.g. Table 11)"
                    value={newTableName}
              ...
              <div style={{ border: '1.5px solid var(--border-color)', borderRadius: '12px', overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14.5px' }}>
                  <thead>
                    <tr style={{ backgroundColor: 'rgba(255,255,255,0.02)', borderBottom: '1.5px solid var(--border-color)' }}>
                      <th style={{ padding: '12px 16px', textAlign: 'left', color: 'var(--text-secondary)', fontWeight: '800' }}>Table Name</th>
                      <th style={{ padding: '12px 16px', textAlign: 'left', color: 'var(--text-secondary)', fontWeight: '800' }}>Floor</th>
                      <th style={{ padding: '12px 16px', textAlign: 'left', color: 'var(--text-secondary)', fontWeight: '800' }}>Status</th>
                      <th style={{ padding: '12px 16px', textAlign: 'center', color: 'var(--text-secondary)', fontWeight: '800' }}>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {(() => {
                      const floorTbls = tablesList.filter(t => t.floor === adminActiveFloor);
                      if (floorTbls.length === 0) {
                        return (
                          <tr>
                            <td colSpan="4" style={{ padding: '30px', textAlign: 'center', color: 'var(--text-secondary)' }}>
                              No tables configured for Floor {adminActiveFloor}.
                            </td>
                          </tr>
                        );
                      }
                      ...
                      return floorTbls.map(t => {
                        const isEditing = editingTableId === t.id;
                        return (
                          <tr key={t.id} style={{ borderBottom: '1.5px solid var(--border-color)' }}>
                            ...
                            <td style={cellStyle}>
                              {isEditing ? (
                                <select
                                  value={editingTableFloor}
                                  onChange={(e) => setEditingTableFloor(Number(e.target.value))}
                                  style={{
                                    padding: '6px 10px',
                                    borderRadius: '6px',
                                    border: '1.5px solid var(--border-color)',
                            ...
                            <td style={cellStyle}>
                              <span style={{
                                ...
                                padding: '3px 8px',
                                borderRadius: '6px',
                                backgroundColor: t.status === 'free' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                                color: t.status === 'free' ? '#10b981' : '#ef4444'
                              }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'center', gap: '6px' }}>
                                  <button
                                    onClick={() => {
                                      ...
                                      if (tablesList.some(item => item.id !== t.id && item.name.toLowerCase() === name.toLowerCase())) {
                                        alert('Table name already exists!');
                                        return;
                                      }
                                      const updated = tablesList.map(item =>
                                        item.id === t.id ? { ...item, name, floor: editingTableFloor } : item
                                      );
                                      setTablesList(updated);
                                      localStorage.setItem('floor_plan_tables', JSON.stringify(updated));
